...
1. **Syntax modernization** — `var`, text blocks, switch expressions, pattern matching, module imports
2. **API upgrades** — `java.time`, `java.nio.file`, `HttpClient`, `List.of`/`Map.of`, `Optional`, `Stream`
3. **Pattern adoption** — records, sealed interfaces, pattern matching for `instanceof` and `switch`
4. **Functional style** — streams over loops, lambdas over anonymous classes, method references
5. **Concurrency modernization** — virtual threads, structured concurrency, scoped values (only when the codebase already uses them or the target JDK supports them)
...
- Use module imports (`import module java.net.http;`) over individual type imports when the target JDK supports them
