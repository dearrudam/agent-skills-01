...
Generic, composable Java 25 code conventions — modern syntax, code style, naming, visibility, structure, methods, streams, exceptions, and documentation rules that apply across all Java contexts (single-file scripts, CLI apps, MicroProfile/Jakarta EE servers, libraries).

Adapted from [Adam Bien's `airails` Java conventions](https://github.com/AdamBien/airails/tree/main/java/java-conventions).
...
- use module imports (e.g., `import module java.net.http;`) over individual type imports, but only for modules other than `java.base`
- do not use `import module java.base;` in ordinary classes — it is overly broad and pulls in every package exported by the module
- `java.base` packages are always accessible, but ordinary classes still need specific type imports (e.g., `import java.util.List;`) or rely on the module import of a non-base module
- compact source files and implicitly declared main classes automatically import all `java.base` public types, so they need no explicit `java.base` import
